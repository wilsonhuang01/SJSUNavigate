...
class MapImage extends Canvas {
	...
	public void paint(Graphics g) {
		...
		Image img = t.getImage(getClass().getResource("images/map.jpg"));
		Image logo = t.getImage(getClass().getResource("images/logo.png"));
        ...
        try {
			BufferedImage bimg = ImageIO.read(getClass().getResource("blocks/greenblock.png"));
    		final ImageIcon greenicon = new ImageIcon(bimg);
			bimg = ImageIO.read(getClass().getResource("blocks/transparentgreenblock.png"));
    		final ImageIcon tgreenicon = new ImageIcon(bimg);
			bimg = ImageIO.read(getClass().getResource("blocks/redblock.png"));
    		final ImageIcon redicon = new ImageIcon(bimg);
			bimg = ImageIO.read(getClass().getResource("blocks/transparentredblock.png"));
    		final ImageIcon tredicon = new ImageIcon(bimg);
			bimg = ImageIO.read(getClass().getResource("blocks/transparentblock.png"));
    		final ImageIcon ticon = new ImageIcon(bimg);
			bimg = ImageIO.read(getClass().getResource("blocks/transparentblueblock.png"));
    		final ImageIcon tblueicon = new ImageIcon(bimg);

			File input = new File(System.getProperty("user.dir") + "/src/main/resources/MapInfo.txt");
...
public class SJSUNav {
	private static final String mapInfo = System.getProperty("user.dir") + "/src/main/resources/MapInfo.txt";
	...
	public static void main(String args[]) {
		...
		try {
			...
			confirm.addMouseListener(new MouseAdapter() 
			{
				...
				{
					mi.showRoute(false);
    				LinkedList<Node> path = Path.findShortestPathByName(mapInfo, startField.getText().toUpperCase(), endField.getText().toUpperCase()).getPath();
		        	...
		        	if (x < 128 && y < 128) {
		        		if (mi.getButtons()[x][y] != null) {
		        			if (mi.hasStart() && mi.hasEnd()) {
		        				...
		        				LinkedList<Node> path = Path.findShortestPathByName(mapInfo, startName, endName).getPath();
		        				System.out.println("LIST OUTPUT: ");
		        				System.out.println(path.toString());
		        				for (int i = 0; i < path.size(); i++) {
		        					if (!(path.get(i).toString().contains("-"))) path.remove(i);
